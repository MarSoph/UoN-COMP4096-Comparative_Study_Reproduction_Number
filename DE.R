...
Optimisation=function(v_value, niter){
  ...
  ode_MSE=function(x, y0=y0, data=Simu){
    ...
    if (v_value==0.013){
      ...
      #generating incidence data from the not noisy simulated data
      #Srep=numeric(151)
...
DE_calc_Rt=function(v_value,data, x){
  ...
  for (t in 1:151){
    ...
    parameters=list(S=1,E=0,P=0, I=0,J=0, A=0, 
                           Tv=0, V=0, Ev=0, Pv=0, 
                           Iv=0, Jv=0, Av=0, R=0, D=0,  #Disease Free Equilibrium 
    ...
    R0[t]=(max(eigen(f %*% solve(vv))$values)) #basic reproduction number
  }
  
  Rt=(data$S + data$Tv+data$V)*R0 #Rt
  Rt=as.data.frame(Rt)
  Rt$t=seq(1:151)
  return(list(R0,Rt))
}
...
## --- Boxplots ---##
### --- v=0.013 --- ###
rates_probs=as.data.frame(subset(DE013res, par %in% c('b1','db2','db3','db4','db5','db6','d','f','g','p','pv','u','uA')))

period_durations=as.data.frame(subset(DE013res, par %in% c('a1_inv','a2_inv','phi_inv','n_inv','k_inv','l_inv')))
...
ggplot(rates_probs, aes(x=par, y=variables, group=par, color=variable)) +
  ...
  scale_x_discrete(labels=c('b1'=expression(beta*1),'db2'=expression(d*beta*2),'db3'=expression(d*beta*3),
                            'db4'=expression(d*beta*4),'db5'=expression(d*beta*5),'db6'=expression(d*beta*6),
                            'd'=expression(delta),'f'=expression(f),'g'=expression(gamma),'p'=expression(p),
                            'pv'=expression(pv),'u'=expression(u),'uA'=expression(uA)))
...
ggplot(period_durations, aes(x=par, y=variables, group=par, color=variable)) +
  ...
  scale_x_discrete(labels=c('a1_inv'= expression(1/alpha*1 ),'a2_inv'= expression(1/alpha*2),
                            'k_inv'=expression(1/kappa),   'phi_inv'=expression(1/phi), 'n_inv'='1/n', 'l_inv'='1/l'))

ggsave(file = "DEparrates013.pdf", width = 7.30,height=3.9, units=c('in'))

### --- v=0.005 --- ###
rates_probs=as.data.frame(subset(DE005res,
                                 par%in%c('b1','db2','db3','db4','db5','db6','d','f','g','p','pv','u','uA')))

period_durations=as.data.frame(subset(DE005res, 
                                      par %in% c('a1_inv','a2_inv','phi_inv','n_inv','k_inv','l_inv')))
...
ggplot(rates_probs, aes(x=par, y=variables, group=par, color=variable)) +
  ...
  scale_x_discrete(labels=c('b1'=expression(beta*1),'db2'=expression(d*beta*2),'db3'=expression(d*beta*3),'db4'=expression(d*beta*4),'db5'=expression(d*beta*5),'db6'=expression(d*beta*6),'d'=expression(delta),'f'=expression(f),'g'=expression(gamma),'p'=expression(p),'pv'=expression(pv),'u'=expression(u),'uA'=expression(uA)))
...
ggplot(period_durations, aes(x=par, y=variables, group=par, color=variable)) +
  ...
  facet_wrap(~Scenario)+
  scale_x_discrete(labels=c('a1_inv'= expression(1/alpha*1 ),'a2_inv'= expression(1/alpha*2),'k_inv'=expression(1/kappa),   'phi_inv'=expression(1/phi), 'n_inv'='1/n', 'l_inv'='1/l'))

ggsave(file = "DEparrates005.pdf", width = 7.30,height=3.9, units=c('in'))

# --- Generating the Incidence data --- #
SEPIJARV_opt=function(t,y,parms){
    a1_inv=parms[1]
    a1=1/a1_inv
    a2_inv=parms[2]
...
estimated_data=function(par, v_value){
  'generates incidence and full epidemiological data
...
#--- Generating the incidence and full data using the optimal parameters
                                                        #given by DE ---#
...
The relative RMSE is `r relRMSE013` and `r relRMSE005` for v=0.013 and v=0.005, respectively.  

#--- Plot the generated and originally simulated incidence data 
                                                          #on the same plot---#
compare_inc_plot=function(inc, v_value){
  'Plots the generated and simulated (original) incidence data 
on the same figure'
  if (v_value==0.013){
  ...
  gg=ggplot(simu,aes(x=Days))+
    geom_line(aes(y=SimulatedInc*888000, linetype='Simulated (True)', 
                  col='Simulated (True)'))+
    geom_line(aes(y=GeneratedInc*888000, linetype='Generated',
                  col='Generated'))+
    scale_color_manual(values=c("Blue","Black"))+
    scale_linetype_manual(values = c(1,2))+
    labs(linetype='Incidence Data')+
    ylab('Daily number of new cases')+
    guides(color=FALSE)+
    ggtitle(paste('Incidence data for',v_value,'vaccination rate'))+
...
The RMSE for v=0.013 is `r RMSE013` and the RMSE for v=0.005 is `r RMSE005` using 
  the number of new cases rather than the proportion of the population being the new cases. 

#---Plot/Compare the generated and the simulated/true incidence data---#
...
DERt013=DE_calc_Rt(v_value=0.013,
                   data=data_out_013[[1]],
                   x=DE013res$Optimal[1:19])[[2]]
DERt013$True=read.csv('Rt_013.csv')$Rt

GRt13=ggplot(DERt013,aes(x=t))+
  geom_line(aes(y=True, linetype='True Values', col='True Values'))+
    geom_line(aes(y=Rt, linetype='Estimated', col='Estimated'))+
    scale_color_manual(values=c("Blue","Black"))+
    scale_linetype_manual(values = c(1,2))+
    labs(linetype='Time-Varying R')+
    xlab('Days')+
    ylab('Time-Varying R')+
    guides(color=FALSE)+
...
DERt005=DE_calc_Rt(v_value=0.005,
                   data=data_out_005[[1]],
                   x=DE005res$Optimal[1:19])[[2]]

DERt005$True=read.csv('Rt_005.csv')$Rt
...
GRt05=ggplot(DERt005,aes(x=t))+
  geom_line(aes(y=True, linetype='True Values', col='True Values'))+
    ...
    theme(legend.key.size = unit(9, 'mm'), 
          ...
          legend.title =element_text(size = unit(9, 'mm')),
          legend.position = 'bottom')
ggsave(file = "DERt005.pdf")

ggarrange(GRt13, GRt05,common.legend = TRUE, legend = 'bottom')

...
plot_R=function(Rt,v, step, noise){
  if (v==0.013){
    original=read.csv('Rt_013.csv')$Rt
    DE=read.csv('DERt013.csv')$Rt
  }
  if (v==0.005){
    original=read.csv('Rt_005.csv')$Rt
  ...
  Bias=mean(abs(Rt$Mean[3:l]-Rt$original[3:l]))
  RMSE=rmse(Rt$original[3:l],Rt$Mean[3:l])
...
SI_Nishiura=c(mu=4.8, sd=2.3) #@Nishiura2020 
SI_Du=c(mu=3.96, sd=4.75) #@Du2020
...
estimate_R_plots(Rt013_D, what='R', type='l')
estimate_R_plots(Rt005_D, what='R', type='l')
estimate_R_plots(Rt013D_D, what='R', type='l')
estimate_R_plots(Rt005D_D, what='R', type='l')
```

```{r}
# given mean SI: 4.8 (95% CrI: 3.8,6.1) @Nishiura2020 given Weibull distribution
# and SD: 2.3 (95% CrI: 1.6, 3.5) 

#Normally distributed SI:
## mean: 3.96 (95% CI: 3.53, 4.39)
## SD: 4.75 (95% CI: 4.46, 5.07) @Du2020
